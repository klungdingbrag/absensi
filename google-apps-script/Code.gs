...
function doGet(e){
  try{
    ...
    if(rows.length===0)return jsonResponse({success:true,data:[],weekStart:weekStart,payrollPayments:getPayrollPayments_(weekStart)});
    ...
    return jsonResponse({success:true,data:data,weekStart:normalizeDateValue_(row[0]),weekEnd:normalizeDateValue_(row[1]),updatedAt:normalizeDateTimeValue_(row[3]),payrollPayments:getPayrollPayments_(weekStart)});
...
function doPost(e){
  ...
  try{
    ...
    const payrollPayments=normalizePayrollPayments_(payload.payrollPayments||payload.payrollPayment);
    ...
    savePayrollPayments_(weekStart,weekEnd,payrollPayments);
    SpreadsheetApp.flush();
    return jsonResponse({success:true,message:'Data berhasil disimpan.',weekStart:weekStart,weekEnd:weekEnd,payrollPayments:payrollPayments});
  }catch(error){return jsonResponse({success:false,message:error.message||String(error)});}
  finally{try{lock.releaseLock();}catch(_) {}}
}

function getPayrollPayments_(weekStart){
  ...
  values.forEach(r=>{
    if(normalizeDateValue_(r[0])!==weekStart)return;
    const employeeId=String(r[2]||'').trim();
    if(!employeeId)return;
    out[employeeId]={status:String(r[3]||'UNPAID').toUpperCase()==='PAID'?'PAID':'UNPAID',paidAt:normalizeDateTimeValue_(r[4])||null,paidAmount:Number(r[5])||0};
  });
  return out;
}

function normalizePayrollPayments_(value){
  const src=value&&typeof value==='object'?value:{};const out={};
  Object.keys(src).forEach(id=>{const p=src[id]||{};out[String(id)]={status:String(p.status||'UNPAID').toUpperCase()==='PAID'?'PAID':'UNPAID',paidAt:p.paidAt||null,paidAmount:Number(p.paidAmount)||0};});
  return out;
}

function savePayrollPayments_(weekStart,weekEnd,payments){
  const ss=SpreadsheetApp.getActiveSpreadsheet();let sheet=ss.getSheetByName(PAYROLL_PAYMENT_SHEET_NAME);
  if(!sheet){sheet=ss.insertSheet(PAYROLL_PAYMENT_SHEET_NAME);sheet.getRange(1,1,1,7).setValues([['WEEK_START','WEEK_END','EMPLOYEE_ID','STATUS','PAID_AT','PAID_AMOUNT','UPDATED_AT']]);sheet.setFrozenRows(1);}
  const lastRow=sheet.getLastRow();
  if(lastRow>1){
    const values=sheet.getRange(2,1,lastRow-1,7).getValues();
    for(let i=values.length-1;i>=0;i--){if(normalizeDateValue_(values[i][0])===weekStart){sheet.deleteRow(i+2);}}
  }
  const rows=Object.keys(payments||{}).map(id=>{const p=payments[id];return [weekStart,weekEnd,id,p.status,p.paidAt?new Date(p.paidAt):'',Number(p.paidAmount)||0,new Date()];});
