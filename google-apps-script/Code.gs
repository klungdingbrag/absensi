...
function getSalaryHistory_(employeeId){
  if(!employeeId)throw new Error('employeeId wajib diisi.');
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  const sheet=ss.getSheetByName(SALARY_HISTORY_SHEET_NAME);
  ...
  const data=values.filter(r=>String(r[1]||'').trim()===employeeId).map(r=>({changedAt:normalizeDateTimeValue_(r[0]),employeeId:String(r[1]),nama:String(r[2]),gajiLama:Number(r[3])||0,gajiBaru:Number(r[4])||0,berlakuMulai:normalizeDateTimeValue_(r[5]),alasan:String(r[6]||'')}));
  data.reverse();
  return jsonResponse({success:true,data:data});
}
...
function saveMaster_(incoming){
  ...
  const clean=incoming.map(k=>({id:String(k.id||'').trim(),nama:String(k.nama||'').trim(),gajiPokok:Number(k.gajiPokok)||0,status:String(k.status||'AKTIF').trim().toUpperCase()}));
  if(clean.some(k=>!k.id||!k.nama||k.gajiPokok<=0))throw new Error('Master karyawan berisi ID, nama, atau gaji yang tidak valid.');
  const ids=clean.map(k=>k.id);if(new Set(ids).size!==ids.length)throw new Error('ID karyawan duplikat.');
  const sheet=getMasterSheet_();
  const old=readMasterMap_(sheet);backupMaster_(sheet);const now=new Date();
  const rows=clean.map(k=>{const prev=old[k.id];const created=prev&&prev.createdAt?prev.createdAt:now;if(prev&&Number(prev.gajiPokok)!==Number(k.gajiPokok))appendSalaryHistory_(k,prev.gajiPokok,now,'Perubahan gaji master');if(!prev)appendSalaryHistory_(k,0,now,'Karyawan baru');return [k.id,k.nama,k.gajiPokok,k.status,created,now];});
  if(sheet.getLastRow()>1)sheet.getRange(2,1,sheet.getLastRow()-1,6).clearContent();
  sheet.getRange(2,1,rows.length,6).setValues(rows);SpreadsheetApp.flush();
  return jsonResponse({success:true,message:'Master karyawan berhasil disimpan.',data:clean});
}

function getMasterSheet_(){
  const ss=SpreadsheetApp.getActiveSpreadsheet();let sheet=ss.getSheetByName(MASTER_SHEET_NAME);
  if(!sheet){sheet=ss.insertSheet(MASTER_SHEET_NAME);sheet.getRange(1,1,1,6).setValues([['EMPLOYEE_ID','NAMA','GAJI_HARIAN','STATUS','CREATED_AT','UPDATED_AT']]);sheet.setFrozenRows(1);}
  return sheet;
}
