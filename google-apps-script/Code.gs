...
function doGet(e) {
  try {
    ...
    return jsonResponse({
      success:true,
      data:data,
      weekStart:normalizeDateValue_(row[0]),
      weekEnd:normalizeDateValue_(row[1]),
      updatedAt:normalizeDateTimeValue_(row[3])
    });
  } catch (error) {
...
function getSheet_() {
  ...
  if (!sheet) throw new Error('Sheet '+SHEET_NAME+' tidak ditemukan. Tidak ada sheet baru yang dibuat agar data tidak salah arah.');
  if (sheet.getLastColumn() < 4) throw new Error('Struktur sheet '+SHEET_NAME+' tidak lengkap.');
  return sheet;
}
...
function findWeekRows_(sheet, weekStart) {
  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) return [];
  const values = sheet.getRange(2,1,lastRow-1,4).getValues();
  const matches = [];
  for (let i=0;i<values.length;i++) {
    if (normalizeDateValue_(values[i][0]) === weekStart) matches.push({rowNumber:i+2, values:values[i]});
  }
  return matches;
}
...
function backupRow_(weekStart, row) {
  ...
  backup.appendRow([new Date(), weekStart, normalizeDateValue_(row[1]), row[2], normalizeDateTimeValue_(row[3]), 'Snapshot sebelum overwrite']);
  SpreadsheetApp.flush();
}
